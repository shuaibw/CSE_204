...
int main() {
    ...
    cout << "Inserting " << pq.size() << " numbers in priority queue takes "
         << float(clock() - time_of_insertion_in_pq) / CLOCKS_PER_SEC << " seconds\n";
    ...
    cout << "Inserting " << h.size() << " numbers in my heap takes "
         << float(clock() - time_of_insertion_in_my_heap) / CLOCKS_PER_SEC
    ...
    cout << "Deleting " << numbers.size() << " numbers from priority queue takes "
         << float(clock() - time_of_deletion_in_pq) / CLOCKS_PER_SEC << " seconds\n";

    clock_t time_of_deletion_in_my_heap = clock();

    for (int i = 0; i < number_of_numbers; i++) {
        h.deleteKey();
    } // deleting the root of the heap. You need to implement this.
    cout << h.size() << endl;

    cout << "Deleting " << numbers.size() << " numbers from my heap takes "
         << float(clock() - time_of_deletion_in_my_heap) / CLOCKS_PER_SEC << " seconds\n";

    heapsort(
            numbers); // You need to implement this function in heap.h. You should use the heap class implemented by you to do this. Hint: the function declaration should be void heapsort(vector<int>&v);
    // Now, "numbers" vector contains the numbers in descending order
    cout << std::is_sorted(numbers.begin(), numbers.end(), [](int a, int b) { return a > b; });
    return 0;
}
